...
async function checkTarget(target) {
  ...
  // A broken mail send (wrong SMTP key, unverified sender, provider outage,
  // etc.) must never take down the check itself — the check result is already
  // recorded above, so worst case here is just a missed email, logged clearly,
  // not a crashed/stuck request.
  try {
    if (wasOk && !isOk) {
      const alertResult = await sendAlert(buildDownAlert(target, result));
      if (!alertResult.sent) console.warn(`[mailer] down alert not sent for ${target.name}: ${alertResult.reason}`);
    } else if (!wasOk && isOk) {
      const alertResult = await sendAlert(buildRecoveredAlert(target, result));
      if (!alertResult.sent) console.warn(`[mailer] recovered alert not sent for ${target.name}: ${alertResult.reason}`);
    }
  } catch (e) {
    console.error(`[mailer] failed to send alert for ${target.name}:`, e.message);
  }
